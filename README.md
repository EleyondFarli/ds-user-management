...
- **Reproducible grading/demo environment.** For a course assignment, this guarantees the exact same setup — same JDK/Node versions, same DB, same wiring — runs identically on the student's machine and the grader's machine, avoiding "works on my machine" issues.
